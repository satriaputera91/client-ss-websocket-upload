				...
				if (json_object_get_boolean(endpoint)) {
					...
					if (intent != NULL && name != NULL) {
						...
						if(strncmp(json_object_get_string(name), "iot",3) == 0) {
							...
							struct json_objcet * jbridge = json_object_object_get(data,"bridge");
							struct json_objcet * jdevice_iot = json_object_object_get(data,"device_iot");
								
							char url[2048];
							char payload[2048];
							/**** Variables for Philips ****/
							char device_id[10];
							char ip_hue[16];
							char username[100];
							if(jbridge != NULL && jdevice_iot != NULL ){
								strcpy(device_id, json_object_get_string(json_object_object_get(jdevice_iot, "device_id")));
								strcpy(ip_hue, json_object_get_string(json_object_object_get(jbridge, "ip_bridge")));
								strcpy(username, json_object_get_string(json_object_object_get(jbridge, "token")));
								
								sprintf(url,"https://%s/api/%s/lights/%s/state",ip_hue,username,device_id);
								
                            					lwsl_user("URL : %s \n",url);

								if(strcmp(json_object_get_string(name), "iotDevicePowerOn") == 0) {
									strcpy(payload,"{\"on\":true,\"sat\":254,\"bri\":254,\"hue\":10000}");	
								} else if(strcmp(json_object_get_string(name), "iotDevicePowerOff") == 0){
									strcpy(payload,"{\"on\":false,\"sat\":254,\"bri\":254,\"hue\":10000}");	
								} else if(strcmp(json_object_get_string(name), "iotDeviceChangeColor") == 0){
								
									struct json_object * xy_array = json_object_object_get(jdevice_iot, "xy");
									int arraylen = json_object_array_length(xy_array);
									float x; float y;	
									x = json_object_get_double(json_object_array_get_idx(xy_array,0));
									y = json_object_get_double(json_object_array_get_idx(xy_array,1));
									sprintf(payload,"{\"on\":true,\"sat\":254,\"bri\":254,\"hue\":10000,\"xy\":[%.3f,%.3f]}",x,y);	
						...
						} else if(strncmp(json_object_get_string(name), "stb", 3) == 0) {
							/*FILE *fp;
					...
					if (ret == 0) {
						...
						if (url != NULL) {
	                        ...
	                        sprintf(res_path, "mpc add %s",json_object_get_string(url));
							system(res_path);
